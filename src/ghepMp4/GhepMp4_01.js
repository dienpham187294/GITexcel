...
function VideoMerger() {
  ...
  const mergeVideos = async (videoList, n) => {
    ...
    try {
      ...
      // Re-encode first video with sync settings
      await ffmpeg.exec([
        "-i",
        "file1.mp4",
        "-c:v",
        ...
        "128k",
        "-vsync",
        "1", // Audio/video sync enforcement
        "-async",
        "1", // Audio sync
        "temp1.mp4",
      ]);
      ...
      // Re-encode second video with careful sync settings
      await ffmpeg.exec([
        "-i",
        "file2.mp4",
        "-c:v",
        ...
        "1",
        "-af",
        "aresample=async=1000", // Additional audio resampling for better sync
        "temp2.mp4",
      ]);
      setMergeProgress(35);
      ...
      // Re-encode outro video with sync settings
      await ffmpeg.exec([
        "-i",
        "outro.mp4",
        "-c:v",
      ...
      // Use re-encoding for more precise merging (NOT using copy)
      ...
      await ffmpeg.exec([
        ...
        "input.txt",
        "-c:v",
        "libx264", // Re-encode video for better sync
        "-c:a",
        "aac", // Re-encode audio for better sync
        "-b:a",
        "128k",
        "-vsync",
